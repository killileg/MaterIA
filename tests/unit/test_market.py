...
def test_get_unique_hs_codes(monkeypatch, tmp_path):
    monkeypatch.setattr(up.IO_PATHS, "GEN_PRODUCTS_FOLDER", tmp_path)
    ...
    monkeypatch.setattr(up, "gen_json_objects", fake_gen_json_objects)
    assert up.get_unique_hs_codes() == {"1234", "5678"}


def test_fetch_trade_data(monkeypatch):
    monkeypatch.setattr(up.C, "TRADE_YEARS", ["2021"])
    monkeypatch.setattr(up.C, "TRADE_TARGET", "250")
    monkeypatch.setattr(up.C, "TRADE_FLOW", "M")
    monkeypatch.setattr(up, "time", types.SimpleNamespace(sleep=lambda *_: None))
...
def test_update_shares(monkeypatch):
    monkeypatch.setattr(up, "get_unique_hs_codes", lambda: {"1234"})
    monkeypatch.setattr(
        up,
    ...
    monkeypatch.setattr(
        up, "estimate_market_shares", lambda df: {"A": 0.67, "RoW": 0.33}
    )
    monkeypatch.setattr(up, "tqdm", lambda x, **kwargs: x)
    ...
    monkeypatch.setattr(up, "update_user_data", fake_update_user_data)
